...
<xsl:template name="process-cell">
	...
	<xsl:element name="{$type}">
		...
		<!-- <xsl:value-of select="text:p"/>  This line has been committed on 16.08.2016, cause it cases to read just one line value on multi lines field , it has been corrected with the following foreach :-->               
                <xsl:variable name="set" select="text:p" />
                <xsl:variable name="counttags" select="count($set)" />
                <xsl:choose><!-- if more than (text:p)-Tag, loop foreach tag, and add the value plus new line using concat function  -->
                    <xsl:when test="$counttags &gt; 1">
